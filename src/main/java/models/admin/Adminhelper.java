	...
	public void Updatedetails(String Dataset) {
		...
		try {
			...
			if (records.equals("1 records found")) {
				...
				Common.assertionCheckwithReport(
						Common.getPageTitle().contains("testing qa / Customers"),
...
//					data.get(Dataset).get("DOB"));
	...
	public void Newsletter() {
		...
		{
			e.printStackTrace();
			report.addFailedLog("Validating the newsletter checkout",
							"newsletter checkbox should be selected", "Successfully newsletter checkout selected",
	...
	public void Address(String Dataset) {
		...
		try
		{
			...
			Common.dropdown("xpath", "//div[@class='admin__field-control']//select[@name='country_id']", Common.SelectBy.TEXT, data.get(Dataset).get("Country"));
			Sync.waitElementPresent("xpath", "//select[@name='region_id']");
			Common.clickElement("xpath", "//select[@name='region_id']");
			Common.dropdown("xpath", "//select[@name='region_id']", Common.SelectBy.TEXT, data.get(Dataset).get("State"));
			...
			Common.textBoxInput("xpath", "//div[@class='admin__field-control']//input[@name='telephone']", data.get(Dataset).get("Phonenumber"));
		...
		{
			e.printStackTrace();
			report.addFailedLog("Validating the address with records",
						"User should be able save address ", "Successfully address has been saved",
	...
	public void Orders(String Dataset) {
		...
		try
		{
			...
			else
			{
			 ...
			 Sync.waitElementInvisible(30, "xpath", "//div[@data-role='spinner' and @style='display: none;']");
			 Common.assertionCheckwithReport(ordernumber.equals(number), "Validating the order number in orders page",
						"User should be able see order number  ", "Successfully order number is dispalyed in orders page",
		...
		{
			e.printStackTrace();
			report.addFailedLog("Validating the order number in orders page",
					"User should be able see order number  ", "Successfully order number is dispalyed in orders page",
	...
	public void Returns(String Dataset) {
		...
		try
		{
			...
			{
				String ordernumber=Common.findElement("xpath", "//input[@name='order_increment_id']").getAttribute("value");
				System.out.println(ordernumber);
				 String number=data.get(Dataset).get("ordernumber");
				 Sync.waitElementInvisible(30, "xpath", "//div[@data-role='spinner' and @style='display: none;']");
				 Common.assertionCheckwithReport(ordernumber.equals(number), "Validating the order number in Returns page",
							"User should be able see order number in Returns page  ", "Successfully order number is dispalyed in Returns page",
	...
	public void Shoppingcart(String Dataset) {
		...
		try
		{
			...
			Common.dropdown("xpath", "//select[@id='website_filter']", Common.SelectBy.TEXT, data.get(Dataset).get("Website"));
			Common.clickElement("xpath", "//input[@name='sku']");
			Common.textBoxInput("xpath", "//input[@name='sku']", data.get(Dataset).get("SKU"));
			Common.actionsKeyPress(Keys.ENTER);
			Sync.waitPageLoad();
			String Shoppingrecord=Common.findElement("xpath", "//div[@class='admin__control-support-text']").getText();
			...
			else
			{
				...
				Common.assertionCheckwithReport(SKU.equals(ShoppingSKU), "Validating the Sku ID in Shopping cart page",
						"User should be able see Sku ID in Shopping cart page  ", "Successfully SKU ID is dispalyed in Shopping cart page",
	...
	public void Whishlist(String Dataset) {
		...
		try
		{
			...
			else
			{
				...
				Common.assertionCheckwithReport(product.equals(name), "Validating the productname in the whishlist page",
						"User should be able see product name in the whishlist page  ", "Successfully productname is dispalyed in the whishlist page",
...
public void Select_taxclass(String dataSet) throws Exception {
	...
	Common.dropdown("xpath", "//select[@name='tax_class']",Common.SelectBy.TEXT, data.get(dataSet).get("taxclass"));
	}catch(Exception e) {
...
public void Customer_segments() {
	...
	try {
		...
		Sync.waitElementInvisible(30, "xpath", "//div[@data-role='spinner' and @style='display: none;']");
		Common.assertionCheckwithReport(
				Common.getPageTitle().contains("Customer Segments"),
				"User navigates to customer segment field",
				"User should navigate to the customers segment fields",
...
public void Add_Save_customer_segments(String dataSet) {
	try {
		Sync.waitPageLoad();
		Common.textBoxInput("xpath", "//input[@id='customersegmentGrid_filter_grid_segment_name']", data.get(dataSet).get("SegmentName"));
		Common.actionsKeyPress(Keys.ENTER);
		Sync.waitPageLoad();
		Thread.sleep(8000);
...
//		System.out.println(Common.getPageTitle());
		...
		Common.assertionCheckwithReport(Common.getPageTitle().contains("New Segment"),
				"User creates New customer segment",
				"User should Navigate to New segment page ",
		...
		Common.dropdown("xpath", "//select[@id='segment_is_active']",Common.SelectBy.TEXT, data.get(dataSet).get("AssignedStatus"));
		
		Common.clickElement("xpath", "//select[@id='segment_apply_to']");
		Common.dropdown("xpath", "//select[@id='segment_apply_to']", Common.SelectBy.TEXT, data.get(dataSet).get("ApplyTo"));
		...
		Common.assertionCheckwithReport(
				...
				"User need to save new customer segment field",
				"User saves the new customer segment",
				"User saved the new customer segment", "unable to save the New Segment");
	        
	} catch (Exception | Error e) {
		e.printStackTrace();
		report.addFailedLog("User need to save customer segment field",
				"User saves the new customer segment",
...
public void Edit_customer_segment(String dataSet) {
    ...
    try {
        ...
        Common.assertionCheckwithReport(Common.getPageTitle().contains(segmentName),
                "User selects to edit customer segment field",
                "User should able to edit the customer segment",
...
//        Common.clickElement("xpath", "//textarea[@id='segment_description']");
        ...
        Common.assertionCheckwithReport(
                ...
                "User should able to save edited customer segment",
                "User saves the Edited customer segment", "Unable to save the Edited Customer segment");

    } catch (Exception | Error e) {
        e.printStackTrace();
        report.addFailedLog("User need to save updated customer segment field",
                "User should able to save updated customer segment",
                "User saves the updated customer segment",
...
//		Common.clickElement("xpath", "//td[text()='"+segmentName+ "']");
		...
		Common.assertionCheckwithReport(Common.getPageTitle().contains(segmentName),
				...
				"User should able to select the customer segment",
				"User selects the customer segment", "user unable to select customer segment");
        ...
        Common.assertionCheckwithReport(
				Common.getPageTitle().contains("Customer Segments"),
				"User need to delete customer segment field",
				"User should able to delete the customer segment",
		...
		report.addFailedLog("User need to delete customer segment field",
				"User should able to delete the customer segment",
				"User deletes the customer segment",
...
public void pages() {
	...
	try {
		...
		Common.assertionCheckwithReport(Common.getPageTitle().equals("Pages / Magento Admin"),
				"Validating content filed page navigation ",
				"after clicking on pages it will navigate page filed ",
				"Successfully navigate to the page filed ",
				"Failed to navigate to the page filed");
...
public void hot_elements()
{
	try
	{
		...
		String hotelements=Common.findElement("xpath", "//li[@id='menu-section-hot_elements']").getAttribute("class");
		
		Common.assertionCheckwithReport(hotelements.equals("active"),
				"Verify the Hot element drop down contents ",
				"after clicking on hot elemnet contents should be appear ",
				"Successfully hot elements contents appeared ",
				"Failed to appear hot elements contents");
	}
	catch(Exception |Error e)
	{
		e.printStackTrace();
...
public void editcontent() {
	...
	try
	{
		...
		String visible=Common.findElement("xpath", "//div[contains(@class,'pagebuilder-options ')]").getAttribute("class");
		...
		Common.assertionCheckwithReport(visible.contains("-options-visible"),
				...
				"Successfully edit option appeared in the pagebuilder", "Failed to appear edit option when we mouse over");
...
public void editpromocontent_image() {
	...
	try
	{
		...
		Common.assertionCheckwithReport(image.equals("Lotusqa.png"),
				...
				"Successfully image uploaded in background image ", "Failed to upload image on the background image");
	...
	catch(Exception |Error e)
	{
		...
		report.addFailedLog("validation the image uploading on content for background image ",
				"Image should be upload for background image",
				"Successfully image uploaded in background image ",
...
public void allbackground(String Dataset) {
	...
	try
	{
		...
		Common.dropdown("xpath", "//select[@name='background_position']", Common.SelectBy.TEXT, data.get(Dataset).get("Backgroud position"));
		Common.clickElement("xpath", "//select[@name='background_attachment']");
		Common.dropdown("xpath", "//select[@name='background_attachment']", Common.SelectBy.TEXT, data.get(Dataset).get("Attachment"));
...
public void savecontent(String Dataset) {
	...
	try
	{
		...
		Common.assertionCheckwithReport(
				savethepage.equals("You saved the page."),
...
public void openwebsite(String Dataset) {
	...
	try
	{
		...
		Common.assertionCheckwithReport(
				Common.getPageTitle().equals("qaFlask"),
...
public void deletepage() {
	...
	try
	{
        ...
        Common.assertionCheckwithReport(
				...
				"after clicking save button it will navigate page filed and message should be displayed",
				"Successfully navigate to page filed and message is dispalyed",
				"Failed to navigate to page filed");
	...
	catch(Exception | Error e)
	{
		...
		report.addFailedLog("Validating the User lands to the hydroflask page",
				"User should able lands on the hydroflask page",
				"Sucessfully User lands on the hydroflask page",
				Common.getscreenShotPathforReport("Failed to navigate to page filed"));
		Assert.fail();
	}
}
...
public void Contentpage() {
	...
	try
	{
		...
		String pagebuilder=Common.findElement("xpath", "//div[contains(@data-bind,'pageBuilderEditButtonClick')]").getAttribute("class");
