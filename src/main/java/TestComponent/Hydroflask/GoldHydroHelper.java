	...
	public void contactUsPage(String dataSet) {
		...
		try {
           ...
           Common.clickElement("xpath", "//input[@id='customerEmail']");
			...
			Common.textBoxInput("xpath", "//input[contains(@data-label,'How can we')]", data.get(dataSet).get("CommetsHydroflask"));
	...
	public void Sticky_Add_to_Cart(String Dataset ) {
		...
		public void socialLinkValidation(String dataSet){
			...
			for(i=0;i<socallinksarry.length;i++){
				...
				e.printStackTrace();
			    ExtenantReportUtils.addFailedLog("Verifying Social link ","click the social links it will navigating to particular page","User unabel to navigate Social page",Common.getscreenShotPathforReport("socialpage"));
			    Assert.fail();
			}
		}
...
//				Common.findElement("xpath", "(//input[@type='file'])[5]").sendKeys(path2);
	...
	public void popular_searches() {
		...
		try {
			...
			List<WebElement> Search=Common.findElements("xpath", "(//div[@id='algolia-cms-block-popular-search'])//h4[@class='c-popular-searches__title']/a");
			System.out.println(Search);
			System.out.println(Search.size());
			for(int i=0;i<Search.size();i++)
			{
			
				List<WebElement> select=Common.findElements("xpath", "(//div[@id='algolia-cms-block-popular-search'])//h4[@class='c-popular-searches__title']/a");
				Sync.waitPageLoad();
				Sync.waitElementPresent(50, "xpath", "(//div[@id='algolia-cms-block-popular-search'])//h4[@class='c-popular-searches__title']/a");
				Thread.sleep(3000);
	...
	public void click_BottlesDrinkware() {
	    ...
	    try {
	    Sync.waitElementPresent("xpath", "//ul[contains(@class,'level0 submenu')]//span[text()=' Bottles & Drinkware']");
	    
	        Common.clickElement("xpath", "//ul[contains(@class,'level0 submenu')]//span[text()=' Bottles & Drinkware']");
	    	Thread.sleep(2000);
	...
	public void click_Bottles() {
	    ...
	    Sync.waitElementPresent("xpath", "//ul[contains(@class,'level0 submenu')]//span[text()=' Bottles']");
	        ...
	        Common.assertionCheckwithReport(pagetitle.contains("Shop Bottles"),
	                ...
	                "update PLP page are displayed after a click on the Bottles",
	...
	public void ClickProduct_Image() {
	    ...
	    Sync.waitElementPresent("xpath", "//h2[@data-bind='text: storefrontLabel']");
	        ...
	        Sync.waitPageLoad();
	        Thread.sleep(2000);
	        Common.clickElement("xpath", "//div[@class='fotorama__thumb fotorama_vertical_ratio fotorama__loaded fotorama__loaded--img']");
	    //    Common.clickElement("xpath", "//span[@class='icon-carousel__right']");
	      //  Common.clickElement("xpath", "//span[@class='icon-carousel__left']");
	        ...
	        String text = Common.findElement("xpath", "//div[@class='fotorama__thumb fotorama_vertical_ratio fotorama__loaded fotorama__loaded--img']").getText();
	        System.out.println(text);
	...
	public void url_color_validation(String Dataset) {
		...
		catch(Exception | Error e)
		{
			...
			ExtenantReportUtils.addFailedLog("validating the color and url in PDP page",
					...
					"Unable to display thee color name in the url", Common.getscreenShot("Failed to display thee color name in the url"));
...
//			Carrers();
	...
	public void see_options(String Dataset) {
		...
		try
		{
			 String seeoption=Common.findElement("xpath", "//fieldset[@class='fieldset m-product-card__cta']//span").getText();
             ...
             if(seeoption.equals("See options"))
 			{
 				...
 				Sync.waitPageLoad();
 				Thread.sleep(4000);
 				String emptymessage=Common.findElement("xpath", "//form[@class='form-wishlist-items']//div//span").getText();
 				Common.assertionCheckwithReport(emptymessage.contains("You have no items in your favorites."), "validating the empty products in my favrioutes page ",
     					"Products should not appear in the my favoritoes page", "Sucessfully product are empty in my favorites page",
	...
	public void Coolers_validation(String Dataset) {
		...
		try
		{
			...
			String breadcrumbs=Common.findElement("xpath", "//p[@class='m-breadcrumb__text']").getText();
			System.out.println(title);
			Common.assertionCheckwithReport(title.contains("Coolers") || breadcrumbs.contains("Coolers") , "verifying the header link for coolers ","user should navigate to the coolers page", "user successfully Navigated to the coolers","Failed to navigate to the coolers page");
