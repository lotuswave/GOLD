...
//			Common.textBoxInput("name", "postcode", data.get(dataSet).get("postcode"));
	...
	public void addDeliveryAddress_Guestuser(String dataSet) throws Exception {
		...
		try {
			Thread.sleep(2000);
			int Popup = Common.findElements("xpath", "//div[@class='modal-overlay fixed inset-0 bg-popup-overlay z-modal' and contains(@x-bind,'freegift')and @style='display: none;']").size();
			if(Popup==0)
			{
...
//			Common.actionsKeyPress(Keys.PAGE_DOWN);
			...
			int Size= Common.findElements("xpath", "//div[@aria-label='Payment Methods']").size();
			if(Size==0) {
				...
				if (Common.getCurrentURL().contains("preprod") || Common.getCurrentURL().contains("stage")) {
					...
					Common.scrollIntoView("xpath", "(//button[contains(text(),'Place Order')])[2]");
...
//			Common.textBoxInput("xpath", "//input[@name='email']", data.get(Dataset).get("UserName"));
	...
	public void Configurable_addtocart_pdp(String Dataset) {
		...
		try {
			...
			{
				System.out.println("Free gift popup displayed");
				Sync.waitElementPresent("xpath", "//div[@x-ref='freegift']//button[@aria-label='Close']");
				Common.clickElement("xpath", "//div[@x-ref='freegift']//button[@aria-label='Close']");
			}
//			for (int i = 0; i <= 10; i++) {
...
//				String klarna1=Common.findElement("xpath", "//h2[@role='status']").getText();
	...
	if(Common.findElements("xpath", "(//div[contains(@class,'checkout-success container')]//p/a)[1]").size()>0) {
		order=Common.getText("xpath", "(//div[contains(@class,'checkout-success container')]//p/a)[1]");
		System.out.println(order);
	}
	if(Common.findElements("xpath","//a[@class='order-number']/strong").size()>0) {
		order=	Common.getText("xpath", "//a[@class='order-number']/strong");
...
//					data.get(dataSet).get("NumberOfUnits"));
	...
	public void Gift_message(String Dataset) {
		...
		try
		{
			...
			Sync.waitElementPresent(40, "xpath", "//span[text()='Update']");
			Common.clickElement("xpath", "//span[text()='Update']");
	...
	public HashMap<String, String> Admin_Order_Details(String orderNumber) {
		...
		try
		{
			...
			System.out.println(Workatostatus);
				 ...
				 Thread.sleep(4000);   
				 Sync.waitElementVisible("xpath", "//span[@id='order_status']");
	...
	public void SelectCustomer_Edit(String dataset) {
		...
		try {
			...
			if (records.equals("1 records found")) {
				...
				Sync.waitPageLoad();
				Thread.sleep(4000);
//				Common.assertionCheckwithReport(Common.getPageTitle().contains("/Customer/"),
